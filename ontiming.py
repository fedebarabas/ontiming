...
def get_psf(x, y, em_rate, exposure, fwhm):
    ...
    if type(A) is np.ndarray:
        return np.array([Ai * np.exp(-4 * np.log(2) * (x - center)**2 / fwhm**2) * 
                        np.exp(-4 * np.log(2) * (y - center)**2 / fwhm**2) for Ai in A])
    
    else:
        return A * np.exp(-4 * np.log(2) * (x - center)**2 / fwhm**2) * np.exp(-4 * np.log(2) * (y - center)**2 / fwhm**2)
...
#windows = np.zeros((trials, t_ons.size, time.size, trc_window_l, trc_window_l))
