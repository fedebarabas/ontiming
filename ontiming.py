...
def dgauss(x, y, A, center, fwhm):
    return np.ceil((A / 15) * np.exp(-(x - center)**2 / (2 * fwhm**2)) * 
                   np.exp(-(y - center)**2 / (2 * fwhm**2)))
...
def ontiming():
    ...
    t_window = 3 * ontime
    time = np.arange(0, t_window.magnitude)
